...
export class WebSocketService {
    ...
    private initializeWebSocketHandlers(): void {
        this.io.on('connection', (socket: Socket) => {
            ...
            socket.on('sendMessage', async (messagePayload: { roomId: string, authorId: number, message: string }) => {
                try {
                    const message = await this.messageService.sendMessage(messagePayload);
                    this.io.to(messagePayload.roomId).emit('newMessage', message); // Broadcast to room
                } catch (error) {
                    console.error("Error sending message via WebSocket:", error);
                    socket.emit('messageError', 'Failed to send message'); // Send error back to sender
                }
            });

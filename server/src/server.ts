...
class Server {
    ...
    constructor(
        ...
        this.webSocketService = new WebSocketService(this.httpServer, messageService);
    ...
    private defineRoutes(): void {
        this.app.post("/rooms", this.asyncHandler(this.handleCreateRoom.bind(this)));
        this.app.get("/rooms", this.asyncHandler(this.handleGetRoom.bind(this)));
        this.app.post("/users", this.asyncHandler(this.handleCreateUser.bind(this)));
        this.app.get("/users", this.asyncHandler(this.handleGetUser.bind(this)));
        this.app.post("/messages", this.asyncHandler(this.handleSendMessage.bind(this)));
        this.app.get("/room-updates", this.asyncHandler(this.handleGetRoomUpdates.bind(this)));
        this.app.get("/room-messages", this.asyncHandler(this.handleGetRoomMessages.bind(this)));
    }
    ...
    private async handleCreateRoom(req: Request, res: Response): Promise<void> {
        await this.roomService.create() // Remove return here, just await the promise
    ...
    private async handleGetRoom(req: Request, res: Response): Promise<void> {
        ...
        if (!room) {
            res.status(404).json({ message: "Room not found" }); // Just call res.status().json()
    ...
    private async handleCreateUser(req: Request, res: Response): Promise<void> {
        ...
        if (!name) {
            res.status(400).json({ message: "Name is required" }); // Just call res.status().json()
            return; // Keep return for early exit
        }
    ...
    private async handleGetUser(req: Request, res: Response): Promise<void> {
        ...
        if (!user) {
            res.status(404).json({ message: "User not found" }); // Just call res.status().json()
            return; // Keep return for early exit
        }
    ...
    private async handleSendMessage(req: Request, res: Response): Promise<void> {
        ...
        if (!roomId || !authorId || !messageText) {
            res.status(400).json({ message: "Room ID, Author ID, and message are required" }); // Just call res.status().json()
            return; // Keep return for early exit
        }
    ...
    private async handleGetRoomUpdates(req: Request, res: Response): Promise<void> {
        const { roomId, lastMessageId } = req.query;
        const messages = await this.messageService.getRoomUpdates(roomId as string, Number(lastMessageId));
        res.status(200).json(messages);
        // Implicitly returns void
    }

    private async handleGetRoomMessages(req: Request, res: Response): Promise<void> {
